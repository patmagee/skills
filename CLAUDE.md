...
- **investigate** — User-invoked only. Keeps the human doing the evaluating, questioning, and synthesizing while Claude gathers evidence and reports it raw, never characterized and never interpreted before the human commits to a reading. Produces one trail file per investigation, with findings by state, append-only evidence records carrying a replayable flag, a ledger of what was never checked, and the human's synthesis verbatim. See `skills/investigate/CLAUDE.md`.
